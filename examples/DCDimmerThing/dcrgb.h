...
#include <rgb_leds.h>
#include <RGBDimmer.h>
using ColorUtils::HSV;
using ColorUtils::RGB;
using ColorUtils::RGBDimmer;

RGBDimmer rgbDimmer;
RGBLed led = RGBLed(PIN_R, PIN_G, PIN_B, LED_IS_CC);
...
public:
    ...
    struct DCRGBDimmerInputs
    {
        ...
        bool wakeUp()
        {
            if (pressButtonCheck() && (rgbDimmer.isOff()))
            {
                rgbDimmer.on();
                return true;
            }
        ...
        int brightness(uint8_t command)
        {
            ...
            case DOOYA_DC::UP_AND_STOP:
                if (slowInputsTimer.check())
                    rgbDimmer.brightnessMax();
                break;
            case DOOYA_DC::DOWN_AND_STOP:
                if (slowInputsTimer.check())
                    rgbDimmer.brightnessMin();
                break;
        ...
        int toneStopChangesMode(uint8_t command)
        {
            ...
            case DOOYA_DC::STOP:
                ...
                break;
            default:
                return toneStopChangesStatus(command);
            }
